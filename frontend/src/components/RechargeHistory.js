...
const RechargeHistory = () => {
  ...
  const rechargeTx = transactions.filter(tx => tx.type === 'recharge');

  return (
    <div className="recharge-history-root" style={{ minHeight: '100vh', background: '#f8fafc', color: '#1e293b', paddingBottom: '40px' }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Poppins:wght@300;400;500;600;700;800&display=swap');
        .recharge-history-root * {
      ...
      <div style={{ background: 'linear-gradient(135deg, #16a34a 0%, #15803d 100%)', color: 'white', padding: '24px 16px', position: 'relative', textAlign: 'center', boxShadow: '0 4px 15px rgba(22, 163, 74, 0.2)' }}>
        ...
        <span style={{ fontSize: '20px', fontWeight: 800, letterSpacing: '0.5px' }}>Recharge History</span>
      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          {rechargeTx.length === 0 ? (
            <div style={{ background: 'white', borderRadius: '20px', padding: '40px 20px', textAlign: 'center', border: '1px solid #e2e8f0', boxShadow: '0 8px 30px rgba(0,0,0,0.02)' }}>
              ...
              <p style={{ margin: 0, fontSize: '15px', fontWeight: 600, color: '#64748b' }}>No recharge requests found.</p>
            </div>
          ) : (
            rechargeTx.map((tx, idx) => (
              <div 
                key={idx} 
                style={{ 
                  display: 'flex', 
              ...
              >
                <div>
                  ...
                  <span style={{ fontSize: '12px', color: '#94a3b8', display: 'block', marginTop: '4px', wordBreak: 'break-all' }}>
